...
for(alg.name in c('sgd', 'asgd', 'implicit')){
  plot.name = paste(alg.name, toString(theta.run), "var", "plot", sep="_")
  print(plot.name)
  pdf(paste(plot.name, "pdf", sep="."))
  load(file.name(alg.name, 1))
  plot(nlist, log10(res$dist), "l", ylim=c(-1.25, -.5), lwd=2, col=colors()[25], ann=FALSE)
  
  for(a.id in 2:length(a.tests)){
    load(file.name(alg.name, a.id))
    lines(nlist, log10(res$dist), lwd=2, col=colors()[(a.id-1)*25])
  }
